...
export const updatePointOfInterestCategory = async (server: FastifyInstance, request: FastifyRequest, reply: FastifyReply) => {
    ...
    //const pointOfInterestCategory = await server.orm.getRepository(PointsOfInterestCategories).update(id, request.body as DeepPartial<PointsOfInterestCategories>);
    //reply.send(pointOfInterestCategory);
};
